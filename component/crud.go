...
func (p *CRUD) Messages(newCRUDMessages string) *CRUD {
	p._NotEmpty("messages", newCRUDMessages)
	p.JsonStr = append(p.JsonStr, fmt.Sprintf(`"messages":[%s]`, newCRUDMessages))
	return p
}
...
func (m *MessagesCRUD) FetchFailed(fetchFailed string) *MessagesCRUD {
	m.FetchFailedMsg = fetchFailed
	return m
}
func (m *MessagesCRUD) SaveOrderFailed(saveOrderFailed string) *MessagesCRUD {
	m.SaveOrderFailedMsg = saveOrderFailed
	return m
}
func (m *MessagesCRUD) SaveOrderSuccess(saveOrderSuccess string) *MessagesCRUD {
	m.SaveOrderSuccessMsg = saveOrderSuccess
	return m
}
func (m *MessagesCRUD) QuickSaveFailed(quickSaveFailed string) *MessagesCRUD {
	m.QuickSaveFailedMsg = quickSaveFailed
	return m
}
func (m *MessagesCRUD) QuickSaveSuccess(quickSaveSuccess string) *MessagesCRUD {
	m.QuickSaveSuccessMsg = quickSaveSuccess
	return m
}
func (m *MessagesCRUD) Build() string {
	s, _ := json.Marshal(&m)
	return string(s)
}

func NewMessagesCRUD() *MessagesCRUD {
	m := new(MessagesCRUD)
	return m
}
